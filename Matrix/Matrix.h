...
Mat& set	(T x, T y);
Mat& set	(T x, T y, T z);
Mat& getCol	(int _col, Mat& a)				// 读/写一列/行
...
/******************************************************************************
	...
	* 索引方向: 先纵再横.
	...
	inline T& operator()(int x, int y) { return data[x * cols + y]; }
	...
	Mat& setBlock(Mat& a, int rowSt, int colSt, int rowEd = -1, int colEd = -1) {
		if(rowEd == -1){
			for (int i = 0; i < a.rows; i++)
				for (int j = 0; j < a.cols; j++)
					(*this)(i + rowSt, j + colSt) = a(i, j);
		} 
		else {
			for (int i = rowSt; i <= rowEd; i++)
				for (int j = colSt; j <= colEd; j++)
					(*this)(i, j) = a(i - rowSt, j - colSt);
		}
		return *this;
	}
